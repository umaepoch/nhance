...
def execute(filters=None):
	...
	creation_date = filters.get("date_of_creation")
	item_code = filters.get("item_code")
	if company is not None and purchase_receipt is not None:
		items_list = get_purchase_receipt_items(company,purchase_receipt,creation_date,item_code)
...
def get_purchase_receipt_items(company,purchase_receipt,creation_date,item_code):
	items_map = {}
	pr_items_list = ""
	if company is not None and purchase_receipt is not None and creation_date is None and item_code is None:
		pr_items_list = frappe.db.sql("""select pr.name as name,pri.item_code as item_code,pri.item_name as item_name,
			     ...
			     `tabPurchase Receipt` pr,`tabPurchase Receipt Item` pri where pr.name=pri.parent and
				 pr.company = %s and pri.parent = %s""", (company,purchase_receipt), as_dict=1)
		print ("pr_items_list---------------",pr_items_list)
	if company is not None and purchase_receipt is not None and creation_date is not None and item_code is None:
		pr_items_list = frappe.db.sql("""select pr.name as name,pri.item_code as item_code,pri.item_name as item_name,
			      pri.qty as qty,pri.stock_uom as stock_uom,pr.posting_date as date from
			     `tabPurchase Receipt` pr,`tabPurchase Receipt Item` pri where pr.name=pri.parent and
				 pr.company = %s and pri.parent = %s and pr.posting_date = %s""", (company,purchase_receipt,creation_date), as_dict=1)
	if company is not None and purchase_receipt is not None and creation_date is not None and item_code is not None:
		pr_items_list = frappe.db.sql("""select pr.name as name,pri.item_code as item_code,pri.item_name as item_name,
			      pri.qty as qty,pri.stock_uom as stock_uom,pr.posting_date as date from
			     `tabPurchase Receipt` pr,`tabPurchase Receipt Item` pri where pr.name=pri.parent and
				 pr.company = %s and pri.parent = %s and pr.posting_date = %s and pri.item_code = %s""", (company,purchase_receipt,creation_date,item_code), as_dict=1)
...
def make_prnfile(ncopies,label):
	...
	for report_data in data_summary:
		...
		for copies in range(total_copies):
