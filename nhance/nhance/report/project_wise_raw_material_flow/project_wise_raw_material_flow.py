...
import ast
import sys
#reload(sys) #verison12
#sys.setdefaultencoding('utf-8') #verison12
...
def get_bom_items(filters):
	...
	if filters.get("include_exploded_items") == "Y":
		#print "in------------Y"
		return frappe.db.sql("""select bo.name as bom_name, bo.company, bo.item as bo_item, bo.quantity as bo_qty, bo.project, bi.item_code, bi.stock_qty as bi_qty from `tabBOM` bo, `tabBOM Explosion Item` bi where bo.name = bi.parent and bo.is_active=1 and bo.docstatus = "1" and bo.company = %s and bi.parent = %s order by bo.name, bi.item_code""",(filters.get("company"),filters.get("bom")), as_dict=1)
	else:
		return frappe.db.sql("""select bo.name as bom_name, bo.company, bo.item as bo_item, bo.quantity as bo_qty, bo.project, bi.item_code, bi.stock_qty as bi_qty from `tabBOM` bo, `tabBOM Item` bi where bo.name = bi.parent and bo.is_active=1 and bo.docstatus = "1" %s order by bo.name, bi.item_code""",(company,bom), as_dict=1)
'''
