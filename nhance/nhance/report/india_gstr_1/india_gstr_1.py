...
class Gstr1Report(object):
			...
			for item_list in self.data:
				...
				if sales_in != self.sales_Invoice_iD:
					...
					for items in invoice_items_data:
						...
						if total_disc == 0:
							total_disc = "0"
						nil_rate_non_gst = items.nil_rated_or_exempt_or_nongst_item
						nil_rate_non_gst_item=""
						if nil_rate_non_gst == 1:
							nil_rate_non_gst_item = "Yes"
						else:
							nil_rate_non_gst_item = "No"
						bill_of_supply = ""
						...
						for invoice_value in self.value_of_invoice_data:
							...
							if self.invoice_ID in sales_invoice_id:
								...
								dealer_registered = invoice_value.composition_dealer_or_uin_restration
								if dealer_registered == 1:
									check_for_registed = "Yes"
								else:
									check_for_registed = "No"
								if bill_of_supply == 1:
						...
						for items_data in tax_data_details:
							...
							if "cess_amount" in items_data:
								...
								cess_tax_amount = "--"
						item_good_service =""
						item_code,is_the_item_is_good_or_serivce = self.get_Item_details()
						if item_code:
							if is_the_item_is_good_or_serivce != None:
								if "S" == str(is_the_item_is_good_or_serivce):
									item_good_service = is_the_item_is_good_or_serivce
								elif "G" == str(is_the_item_is_good_or_serivce):
									item_good_service = is_the_item_is_good_or_serivce
							else :
								item_good_service ="-"
						city = ""
						...
						if self.customer_type == "Company":
							if self.filters.get("type_of_business") ==  "B2B":
								...
								nil_rate_non_gst_item,item_list[25],export_type,port_code,
	...
	def get_invoice_data(self):
		...
		invoice_data = frappe.db.sql("""
			select
				...
				composition_dealer_or_uin_restration,posting_date,docstatus,shipping_bill_number,
	...
	def get_Item_details(self):
		...
		if self.item_code:
			item_code,is_the_item_is_good_or_serivce = frappe.db.get_value('Item',self.item_code,
			['item_code',"is_the_item_is_good_or_serivce"])
		return item_code,is_the_item_is_good_or_serivce
