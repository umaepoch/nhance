...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
	...
	for inv in invoice_list:
		...
		row = [
			inv.name, inv.manual_serial_number,inv.posting_date, inv.customer
		]

		if additional_query_columns:
			for col in additional_query_columns:
				row.append(inv.get(col))
		gstrin_number = address_gstin(inv.customer_address)
		row +=[
			gstrin_number, company_currency,inv.total_qty
		]
		# map income values
		base_net_total = 0
...
def get_columns(invoice_list, additional_table_columns):
	...
	columns = [
		_("Invoice") + ":Link/Sales Invoice:120", _("Manual Serial Number ") + "::130", _("Posting Date") + ":Date:80",
...
def get_invoices(filters, additional_query_columns):
	...
	return frappe.db.sql("""
		select name, posting_date, debit_to, project, customer,manual_serial_number,total_qty,
		customer_name, owner, remarks, territory, tax_id, customer_group,customer_gstin,customer_address,
...
def address_gstin(customer_address):
	gstin_num  = frappe.db.get_value('Address',customer_address,
			['gstin'])
	return gstin_num
