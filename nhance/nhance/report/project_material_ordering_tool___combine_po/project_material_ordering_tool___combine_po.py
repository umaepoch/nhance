...
def execute(filters=None):
	...
	if filters.get("project"):
		...
		doc_project_list = project_list(projects)
			...
			if items_map:
				for (sreq_no) in sorted(items_map):
					...
					for sreq_dict in data:
						...
						pre_purchase_order_details = get_pre_purchase_order(projects,item_code)
						project_being  = get_project_being_order(item_code,projects)
						approved_pre_purchase_order_details = get_approved_pre_purchase_order(projects,item_code)
						...
						'''
						for pre_pur in pre_purchase_order_details:
							if pre_pur.recommended_qty != 0.0:
								recommended_qty += float(pre_pur.recommended_qty)
							else:
								required_qty += float(pre_pur.total_stock_qty)
						for app_pur in approved_pre_purchase_order_details:
							for ap in app_pur:
								if ap.approved_qty != 0.0:
									approved_qty += float(ap.approved_qty)
								elif ap.recommeded_qty !=0.0:
									approved_qty += float(ap.recommended_qty)
								else:
									approved_qty += float(ap.total_stock_qty)
						'''
...
def make_purchase_orders(po_items,project_filter):
	...
	if po_items:
		...
		for items in po_items:
			...
			supplier = ""
			if items_supplier:
				supplier = items_supplier[0]['default_supplier']
			conversion_factor = items['conversion_factor']
			stock_qty = items['po_qty']
			qty_in_puom = float(stock_qty)/float(conversion_factor)
			innerJson_Transfer ={
				"creation": creation_Date,
				"qty": items['po_qty'],
...
def get_report_data(project_filter,swh_filter):
	...
	if doc_project_list:
		for project in doc_project_list:
			...
			items_map = fetch_pending_sreqnos(project['project'],swh_filter)
			...
			sreq_qty = rows[3]
			last_n_highest = rows[15]
			last_n_lowest = rows[16]
			last_n_average = rows[17]
			no_of_transaction = rows[14]
			#mt_qty = float(sreq_qty_in_stock_uom) - float(excess_to_be_ordered)
...
def project_list(project):
	project = frappe.get_list("Project Childs", filters = {"parent":project}, fields=['project'])
	return project
def get_pre_purchase_order(project,item_code):
	pre_purchase = frappe.db.sql(""" select pri.recommended_qty, pri.total_stock_qty, pri.item,pr.project_being_ordered from `tabPre Purchase Orders` pr, `tabPre Purchase Item` pri where pr.project_being_ordered = %s and pr.name = pri.parent and pri.item= %s and pr.docstatus =1""",(project,item_code), as_dict=1)
	return pre_purchase

def get_approved_pre_purchase_order(project,item_code):
	pre_purchase = frappe.db.sql(""" select name from `tabPre Purchase Orders`  where project_being_ordered = %s and docstatus=1 """, (project),as_dict=1)
	approved_qty = []
	for pre in pre_purchase:
		name = pre.name
		approved_pre_purchase = frappe.db.sql(""" select api.total_stock_qty ,api.recommended_qty, api.approved_qty from `tabApproved Pre Purchase Order` ap , `tabApproved Pre Purchase Item` api where ap.pre_purchase_orders = %s and ap.name = api.parent and api.item_code =%s and ap.docstatus=1""",(name,item_code), as_dict=1)
