...
/* eslint-disable */
var flag = false;
frappe.query_reports["Project Material Ordering Tool - Combine PO"] = {
    "filters": [{
        "fieldname": "project",
        "label": __("Project"),
        "fieldtype": "Link",
        ...
        "fieldname": "dont_order_qty",
        "label": __(" Do not order Quantities Less than 1"),
        "fieldtype": "Check"
    }],
    onload: function(report) {
        //console.log("Make PO and Transfer Existing/Required Quantity..........");
        
	 report.page.add_inner_button(__("Make  Pre Purchase Order Report"), function() {
			//console.log("hello clicked---------"+cur_frm.doc.name);
			 var reporter = frappe.query_reports["Project Material Ordering Tool - Combine PO"];
			    var filters = report.get_values();
			    
			    var project_filter = frappe.query_report.get_filter_value("project");
			   // make_report(project_filter)
		          // frappe.route_options = {"Project Being Ordered": project_filter};
			   frappe.set_route("query-report", "Pre Purchase Orders Report",{"project": project_filter});
		});
    },
    /*make_pre_purchase_orders:function(report){
	make_pre_purchase_orders(report);
...
function validate_project_details(project, query_report) {
    var project_list = get_project_list(project);
    //console.log("project liste--------"+project_lsit);
    for(var i =0; i < project_list.length; i++){
	    //console.log("project ------------"+project_list[i].project)
	    frappe.call({
		method: 'frappe.client.get_value',
		args: {
		    doctype: "Project",
		    filters: {
		        name: ["=", project_list[i].project]
		    },
...
/*
	...
	var projects = reportData[i]['project'];
        ...
        /*if (mt_qty > 0) {
            if (dont_transfer_order == 1 && mt_qty >= 1) {
                ...
                if (min_max_item == 0) {
                    ...
                    materialItems['project'] = projects;
...
function fetch_conversion_factor(item_code, purchase_uom) {
    var cf = 0.0;
    frappe.call({
        method: "nhance.nhance.report.project_material_ordering_tool___combine_po.project_material_ordering_tool___combine_po.fetch_conversion_factor",
        args: {
            "parent": item_code,
            "uom": purchase_uom
        },
...
function makePO(sreq_no, supplier, po_items) {
    ...
    frappe.call({
        method: "nhance.nhance.report.project_material_ordering_tool___combine_po.project_material_ordering_tool___combine_po.make_purchase_orders",
        args: {
            "sreq_no": sreq_no,
            "supplier": supplier,
            "po_items": po_items
...
function makeMaterialTransfer(sreq_no, mt_list) {
    frappe.call({
        method: "nhance.nhance.report.project_material_ordering_tool___combine_po.project_material_ordering_tool___combine_po.make_stock_entry",
        args: {
            "sreq_no": sreq_no,
            "mt_list": mt_list
        },
...
function getItemLastPurchasePrice(item_code, po_uom) {
    ...
    frappe.call({
        method: "nhance.nhance.report.project_material_ordering_tool___combine_po.project_material_ordering_tool___combine_po.fetch_last_purchase_price",
        args: {
            "item_code": item_code,
            "uom": po_uom
        },
...
function getItemValuationRateFromStockBalance(item_code) {
    var rate = 0.0;
    frappe.call({
        method: "nhance.nhance.report.project_material_ordering_tool___combine_po.project_material_ordering_tool___combine_po.fetch_stock_balance_valuation_rate",
        args: {
            "item_code": item_code
        },
        async: false,
...
function getItemValuationRateFromPriceList(item_code, price_list_by) {
    var rate = 0.0;
    frappe.call({
        method: "nhance.nhance.report.project_material_ordering_tool___combine_po.project_material_ordering_tool___combine_po.fetch_valuation_rate_from_item_price",
        args: {
            "item_code": item_code,
...
function getItemPriceBasedOn() {
    var price_details = {};
    frappe.call({
        method: "nhance.nhance.report.project_material_ordering_tool___combine_po.project_material_ordering_tool___combine_po.fetch_item_price_settings_details",
        async: false,
        callback: function(r) {
...
function getReportData(project_filter, swh_filter) {
    var reportData = [];
    frappe.call({
        method: "nhance.nhance.report.project_material_ordering_tool___combine_po.project_material_ordering_tool___combine_po.get_report_data",
        args: {
...
function get_supplier_field(doc_name) {
    var reportData = 0;
    frappe.call({
        method: "nhance.nhance.report.project_material_ordering_tool___combine_po.project_material_ordering_tool___combine_po.fields",
        args: {
            "doc_name": doc_name
        },
        async: false,
...
function validate_qty(sreq_no, supplier_items, check_args) {
    ...
    frappe.call({
        method: "nhance.nhance.report.project_material_ordering_tool___combine_po.project_material_ordering_tool___combine_po.check_and_update",
        args: {
            "data": data,
            "sreq_no": sreq_no
        },
...
function getitems_minmax(item_code) {
    var purchase_uom = "";
    frappe.call({
        method: 'frappe.client.get_value',
        args: {
            ...
            fieldname: ["pch_minmax_item"]
        },
        async: false,
        callback: function(r) {
            if (r.message) {
                purchase_uom = r.message.pch_minmax_item;
            } else {
                purchase_uom = null;
            }
        }
...
function get_project_list(project){
	var reportData = 0;
    frappe.call({
        method: "nhance.nhance.report.project_material_ordering_tool___combine_po.project_material_ordering_tool___combine_po.project_list",
        args: {
            "project": project
        },
        async: false,
