...
def execute(filters=None):
	...
	print ("item group ---------------",item_group)
	if item_group is not None:
		items_list = get_master_items(item_group,item_name)
...
def get_master_items(item_group,item_name):
	master_items_list = ""
	if item_name is  None:
		master_items_list = frappe.db.sql("""select
													item_code,item_name,stock_uom,item_group
											from
													`tabItem`
											where
													 item_group = %s""", item_group, as_dict=1)
	elif item_name is not None:
		master_items_list = frappe.db.sql("""select item_code,item_name,stock_uom,item_group from `tabItem` where item_code is not NULL and item_group = %s and item_name = %s""", (item_group,item_name), as_dict=1)
	#print "master_items_list-=------------------------"
...
def make_prnfile(ncopies,label,date_of_import):
	...
	for report_data in data_summary:
		...
		for copies in range(int(ncopies)):
