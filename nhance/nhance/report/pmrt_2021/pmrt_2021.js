...
var col_data= [];
var company = "";
frappe.query_reports["PMRT 2021"] = {
	"filters": [
		{
			"fieldname": "company",
			"label": __("Company"),
    ...
    },
		{
			...
			"on_change": function(query_report) {
											...
											frappe.call({
													method: "nhance.nhance.report.pmrt_2021.pmrt_2021.fetch_project_details",
													args: {
													"project":project
													},
													async: false,
						...
						function() {
								...
								frappe.call({
										...
										callback: function(r)
										{
																	...
																	console.log(" typeof COl data::"+ typeof(col_data));
																	}//end of if..
										}//end of call-back function..
								});//end of frappe call..
								  ...
								  var short_qty_flag="false"
									...
									for (i = 0; i < col_data.length; i++) {
										var short_qty = parseFloat (col_data[i].short_qty)
								...
								if (short_qty_flag == "true"){
									...
									frappe.throw("Short qty for all items are empty,Cannot make StockRequistion");
								}
        ...
        title: __("This Tool can only be used to Request Quantities that have not been requested as per BOM " +project_details['master_bom']+ " for delivery to Warehouse "+project_details['reserve_warehouse']+" and as represented in Column Short Quantity. If you have to request for additional quantities, please change the BOM to reflect additional quantities or use the Stock Requisition Tool Directly"),
				...
				primary_action : function() {
					...
					if (filters.project && project_details['reserve_warehouse'] ) {
						return frappe.call({
                    method: "nhance.nhance.report.pmrt_2021.pmrt_2021.make_stock_requisition",
                    args: {
...
function get_workflowStatus(col_data)
{
	workflowStatus = ""
	frappe.call({
			method: "nhance.nhance.report.pmrt_2021.pmrt_2021.get_workflowStatus",
			args: {
				"master_bom":project_details.master_bom,
				"col_data" :col_data
			},
