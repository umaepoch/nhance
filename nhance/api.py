...
@frappe.whitelist()
def make_stock_entry(materialItemsIssue,mterialItemsReceipt,company):
	materialItemsIssue=eval(materialItemsIssue)
	mterialItemsReceipt=eval(mterialItemsReceipt)
	basic_rate = 0
	ret = ""
...
@frappe.whitelist()
def fetch_Records(parent):
	print "parent********************",parent
	records = frappe.db.sql("""select item_code,qty,parent,warehouse,received_qty,rate from `tabPurchase Order Item` where parent=%s""",(parent) , as_dict=1)	
